...
function SelectOption(props: SelectOptionProps) {
  ...
  const { option, toggleOption } = props;
  ...
  const handleClick = useCallback(
    function handleClick(_e: React.MouseEvent<HTMLLIElement>) {
      toggleOption(option.value);
    },
    [option.value, toggleOption]
  );

  const handleFocusBlur = useCallback((e: React.FocusEvent<HTMLLIElement>) => {
    if (e.type === "blur") {
    }
  }, []);
